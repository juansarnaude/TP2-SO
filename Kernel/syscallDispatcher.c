...
static uint64_t sys_read(unsigned int fd, char *output, uint64_t count)
{
    ...
    if (pcb->lastFd <= fd || pcb->fileDescriptors[fd].mode != READ)
        return 0;
...
{
    PCB *pcb = getProcess(getCurrentPid());
    if (pcb->lastFd <= fd || pcb->fileDescriptors[fd].mode != WRITE)
        return;
    if (fd == STDOUT && pcb->fileDescriptors[fd].pipe == NULL)
    {
...
>>>>>>> Stashed changes
    ...
    return pipeWrite(pcb->fileDescriptors[fd].pipe, buffer, count);
}
...
static int sys_close(int fd)
{
    ...
    if (fd >= pcb->lastFd || pcb->fileDescriptors[fd].mode == CLOSED)
    {
        return;
    }

    if (pcb->fileDescriptors[fd].pipe != NULL)
    {
        pipeClose(pcb->fileDescriptors[fd].pipe);
    }
    pcb->fileDescriptors[fd].mode = CLOSED;

    int lastFd = pcb->lastFd;
    while (lastFd > 0 && pcb->fileDescriptors[lastFd - 1].mode == CLOSED)
    {
        lastFd--;
    }
    pcb->lastFd = lastFd;
